...
```mermaid
graph TB
    subgraph Frontend["Frontend (SolidJS + TypeScript)"]
        UI[UI Components<br/>Charts / Animation / Controls]
        Stores[Signal Stores<br/>simulation / settings / history]
        APIAdapter[API Adapter Layer<br/>Auto-detect environment]
        UI --> Stores
        Stores --> APIAdapter
    end

    subgraph DesktopMode["Desktop Mode"]
        TauriIPC[Tauri IPC<br/>invoke/commands]
        TauriApp[Tauri App<br/>src-tauri/]
    end

    subgraph WebMode["Web Mode"]
        HTTPAPI[HTTP REST API<br/>POST /api/simulate<br/>POST /api/export/*]
        AxumServer[Axum Server<br/>camforge-server]
    end

    subgraph Core["camforge-core (Shared Rust Crate)"]
        Motion[motion.rs<br/>运动规律计算]
        Profile[profile.rs<br/>轮廓计算]
        Geometry[geometry.rs<br/>几何分析]
        Simulation[simulation.rs<br/>完整仿真管道]
        Types[types.rs<br/>CamParams / SimulationData]
    end

    APIAdapter -->|isTauri=true| TauriIPC
    APIAdapter -->|isTauri=false| HTTPAPI
    TauriIPC --> TauriApp
    HTTPAPI --> AxumServer
    TauriApp --> Core
    AxumServer --> Core
    ...
    style DesktopMode fill:#f3e5f5,stroke:#7b1fa2
    style WebMode fill:#fff3e0,stroke:#ef6c00
